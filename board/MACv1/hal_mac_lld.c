 ...
 * @notapi
 */
void mii_write(uint32_t phyaddr, uint32_t reg, uint32_t value) {
  ...
  while ((ETH->MACMIIAR & ETH_MACMIIAR_MB) != 0)
    ;
  /* Dummy read */
  ETH->MACMIIAR = phyaddr | (reg << 6) | MACMIIDR_CR | ETH_MACMIIAR_MB;
  while ((ETH->MACMIIAR & ETH_MACMIIAR_MB) != 0)
    ;
}
 ...
 * @notapi
 */
uint32_t mii_read(uint32_t phyaddr, uint32_t reg) {
...
static void mii_find_phy(MACDriver *macp) {
 ...
 do {
    ...
    for (i = 0U; i < 31U; i++) {
      ...
      if ((mii_read(macp->phyaddr, MII_PHYSID1) == (BOARD_PHY_ID >> 16U)) &&
...
//  mii_write(ETHD1.phyaddr, MII_BMCR, BMCR_RESET);
#if defined(BOARD_PHY_RESET_DELAY)
  osalSysPolledDelayX(BOARD_PHY_RESET_DELAY);
#endif
  while (mii_read(ETHD1.phyaddr, MII_BMCR) & BMCR_RESET)
    ;
#endif

#if STM32_MAC_ETH1_CHANGE_PHY_STATE
  /* PHY in power down mode until the driver will be started.*/
  mii_write(ETHD1.phyaddr, MII_BMCR, mii_read(ETHD1.phyaddr, MII_BMCR) | BMCR_PDOWN);
#endif
...
void mac_lld_start(MACDriver *macp) {
  ...
  mii_write(macp->phyaddr, MII_BMCR, mii_read(macp->phyaddr, MII_BMCR) & ~BMCR_PDOWN);
#endif

  mii_write(macp->phyaddr, 1, 0x3b);
...
bool mac_lld_poll_link_status(MACDriver *macp) {
  ...
  /* this will need rewrite. Now just hardcode things */
